...
void Dictionary::add(const std::string& w) {
  vector<string> items;
  utils::split(w, items, '#');
  if (items.size() != args_->num_field) {
    std::cerr << "\radd the field size of word" << w << 
        <<" is not " << args_->num_field << std::flush;
    return;
  }
  ...
  if (word2int_[h] == -1) {
    ...
    words_.push_back(e);
    word2int_[h] = size_++;
    e.field_words.swap(items);
  } else {
    words_[word2int_[h]].count++;
  }
}

// add field word to words_
void Dictionary::addSubWordsV2(entry &item) {
  vector<string> &field_words = item.field_words;
  vector<int> &subwords = item.subwords;
  for (size_t i=0; i<field_words.size(); ++i) {
      string &w = field_words[i];
      int32_t h = find(w);
      int32_t id = -1;
      if (word2int_[h] == -1) {
        entry e;
...
int32_t Dictionary::getLine(
  ...
  while (readWord(in, token)) {
    utils::split(token, items, '#');
    if (!items.empty()) {
        continue;
    }

    int32_t h = find(items[0]);
